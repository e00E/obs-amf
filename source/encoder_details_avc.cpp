...
std::vector<std::unique_ptr<Setting>> settings() noexcept {
  std::vector<std::unique_ptr<Setting>> result;
  result.emplace_back(new EnumSetting{
      "usage",
      "Usage",
      AMF_VIDEO_ENCODER_USAGE,
      {{AMF_VIDEO_ENCODER_USAGE_TRANSCONDING, "Transcoding"},
       {AMF_VIDEO_ENCODER_USAGE_ULTRA_LOW_LATENCY, "Ultra Low Latency"},
       {AMF_VIDEO_ENCODER_USAGE_LOW_LATENCY, "Low Latency"},
       {AMF_VIDEO_ENCODER_USAGE_WEBCAM, "Webcam"}},
      0});
  result.emplace_back(new EnumSetting{
      "profile",
      "Profile",
      ...
      {{AMF_VIDEO_ENCODER_PROFILE_BASELINE, "Baseline"},
       ...
       {AMF_VIDEO_ENCODER_PROFILE_CONSTRAINED_BASELINE, "Constrained Baseline"},
       {AMF_VIDEO_ENCODER_PROFILE_CONSTRAINED_HIGH, "Constrained High"}},
      1});
  result.emplace_back(new EnumSetting{"tier",
                                      "Tier",
  ...
  result.emplace_back(new IntSetting{
      "target bit rate", "Target Bit Rate", AMF_VIDEO_ENCODER_TARGET_BITRATE, 1,
      std::numeric_limits<int>::max(), 10000000});
  result.emplace_back(new IntSetting{
      "peak bit rate", "Peak Bit Rate", AMF_VIDEO_ENCODER_PEAK_BITRATE, 1,
      std::numeric_limits<int>::max(), 30000000});
  result.emplace_back(
      new BoolSetting{"skip frame enable", "Skip Frames for Rate Control",
                      AMF_VIDEO_ENCODER_RATE_CONTROL_SKIP_FRAME_ENABLE, false});
  result.emplace_back(new IntSetting{"min qp i", "Minimum I Frame QP",
                                     AMF_VIDEO_ENCODER_MIN_QP, 0, 51, 18});
  result.emplace_back(new IntSetting{"max qp i", "Maximum I Frame QP",
                                     AMF_VIDEO_ENCODER_MAX_QP, 0, 51, 46});
  result.emplace_back(new IntSetting{"qp i", "CQP: I Frame QP",
                                     AMF_VIDEO_ENCODER_QP_I, 0, 51, 26});
  result.emplace_back(new IntSetting{"qp p", "CQP: P Frame QP",
                                     AMF_VIDEO_ENCODER_QP_P, 0, 51, 26});
  result.emplace_back(new IntSetting{"qp b", "CQP: B Frame QP",
                                     AMF_VIDEO_ENCODER_QP_B, 0, 51, 26});
  result.emplace_back(new IntSetting{
      "vbv buffer size", "VBV Buffer Size", AMF_VIDEO_ENCODER_VBV_BUFFER_SIZE,
      1, std::numeric_limits<int>::max(), 20000000});
  result.emplace_back(new IntSetting{
      "initial vbv buffer fullness", "Initial VBV Buffer Fullness",
      AMF_VIDEO_ENCODER_INITIAL_VBV_BUFFER_FULLNESS, 0, 64, 64});
  result.emplace_back(new BoolSetting{"enforce hrd",
                                      "Enforce Hypothetical Reference Decoder",
  ...
  result.emplace_back(new IntSetting{"b pic delta qp", "B Frame Delta QP",
                                     ...
                                     4});
  result.emplace_back(
      new IntSetting{"ref b pic delta qp", "Reference B Frame Delta QP",
                     AMF_VIDEO_ENCODER_REF_B_PIC_DELTA_QP, -10, 10, 2});
  result.emplace_back(new IntSetting{
      "intra refresh num mbs per slot", "Intra Refresh Macroblocks Per Slot",
      AMF_VIDEO_ENCODER_INTRA_REFRESH_NUM_MBS_PER_SLOT, 0,
      std::numeric_limits<int>::max(), 0});
  result.emplace_back(new IntSetting{"b pic pattern", "B Frame Pattern",
                                     AMF_VIDEO_ENCODER_B_PIC_PATTERN, 0, 3, 3});
  result.emplace_back(
      new BoolSetting{"b reference enable", "Use B Frames As References",
                      AMF_VIDEO_ENCODER_B_REFERENCE_ENABLE, true});
  */
  result.emplace_back(
      new IntSetting{"header insertion spacing", "Header Insertion Spacing",
                     AMF_VIDEO_ENCODER_HEADER_INSERTION_SPACING, 0, 1000, 0});
  result.emplace_back(new IntSetting{
      "idr period", "IDR Period", AMF_VIDEO_ENCODER_IDR_PERIOD, 0, 1000, 30});
  result.emplace_back(new IntSetting{"slices per frame", "Slices Per Frame",
                                     AMF_VIDEO_ENCODER_SLICES_PER_FRAME, 1,
                                     std::numeric_limits<int>::max(), 1});
  result.emplace_back(new BoolSetting{"deblocking filter", "Deblocking Filter",
                                      AMF_VIDEO_ENCODER_DE_BLOCKING_FILTER,
                                      true});
  result.emplace_back(
      new EnumSetting{"cabac enable",
                      "CABAC",
                      ...
                      0});
  result.emplace_back(new EnumSetting{"scantype",
                                      "Scan Type",
                                      AMF_VIDEO_ENCODER_SCANTYPE,
                                      {{0, "Progressive"}, {1, "Interlaced"}},
                                      0});
  result.emplace_back(
      new EnumSetting{"quality preset",
                      "Quality Preset",
...
const EncoderDetails encoder_details_avc{
    .amf_name = AMFVideoEncoderVCE_AVC,
    .extra_data = AMF_VIDEO_ENCODER_EXTRADATA,
    .frame_rate = AMF_VIDEO_ENCODER_FRAMERATE,
    .input_color = {.profile = AMF_VIDEO_ENCODER_INPUT_COLOR_PROFILE,
                    .transfer_characteristic =
                        AMF_VIDEO_ENCODER_INPUT_TRANSFER_CHARACTERISTIC,
                    .primaries = AMF_VIDEO_ENCODER_INPUT_COLOR_PRIMARIES},
    .output_color = {.profile = AMF_VIDEO_ENCODER_OUTPUT_COLOR_PROFILE,
                     .transfer_characteristic =
